...
QuietBlock's own code is MIT — see [LICENSE](LICENSE).

Three things here are not QuietBlock's own work, and none of them are bundled:

- **EasyList and EasyPrivacy** are third-party filter lists published at
  [easylist.to](https://easylist.to/) and used under
  [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/). QuietBlock
  downloads them at runtime from the URLs configured in the options page; this
  repository contains neither their text nor any derivative of it. They are
  opt-in defaults, and disabling or removing them leaves the extension working.
- **The built-in starter list** (`src/shared/filter/starterList.ts`) is a
  hand-picked set of well-known advertising hostnames. Hostnames are not
  copyrightable, and the list is kept short and conservative on purpose.
- **The icons** are generated by `scripts/gen-icons.mjs` at build time from a
  signed-distance-field renderer written for this project. No third-party
  artwork is used.

Development dependencies (Vite, TypeScript, `@types/chrome`, `@types/node`) are
installed from npm and are not redistributed here; each carries its own licence.

QuietBlock is an independent personal project. It is not affiliated with,
endorsed by, or derived from easylist.to, EasyList, EasyPrivacy, Adblock Plus,
or any commercial ad blocker. The filter-list syntax it parses follows the
Adblock Plus conventions, which are a de-facto format rather than a product.
